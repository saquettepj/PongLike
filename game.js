...
class Game {
  ...
  setupEventListeners() {
    ...
    document.addEventListener("keydown", (e) => {
      ...
      const modifierNotification = document.getElementById(
    ...
    document.addEventListener("keyup", (e) => {
      ...
      if (
        ...
        if (
          this.hasUpgrade("paddle_dash") &&
          this.activeUpgradeEffects.paddleDash.active
        ) {
          this.activeUpgradeEffects.paddleDash.active = false;
          // Iniciar cooldown quando o dash é desativado manualmente
          this.activeUpgradeEffects.paddleDash.timer =
            this.activeUpgradeEffects.paddleDash.cooldown;
        }
      }
      ...
      if (e.code === "Space" && this.gameRunning && !this.gamePaused) {
        if (
          ...
          this.activeUpgradeEffects.dimensionalBall.active
        ) {
    ...
    document
      .getElementById("confirmPowerButton")
      .addEventListener("click", () => {
        this.confirmInitialPower();
      });
    const confirmPowerButtonMobile = document.getElementById(
      "confirmPowerButtonMobile",
    );
    ...
    const continueButtonMobile = document.getElementById(
  ...
  confirmInitialPower() {
    ...
    const allUpgrades = this.getAllUpgrades();
    const selectedUpgrade = allUpgrades.find(
      (upgrade) => upgrade.id === this.selectedInitialPower,
    );
  ...
  removeInitialPower() {
    ...
    this.activeUpgrades = this.activeUpgrades.filter(
      (upgrade) => upgrade.id !== this.initialPower.id,
    );
  ...
  startResumeCountdown() {
    ...
    const tick = () => {
      ...
      numberEl.style.animation =
        "titleGlow 2s ease-in-out infinite alternate, countdownPop 0.6s ease forwards";
  ...
  initGameObjects() {
    // Criar paddle
    const paddleWidth =
      this.config.paddleWidth * this.difficultySettings.paddleSizeMultiplier;
    this.paddle = {
  ...
  generateBricks() {
    ...
    const cols = Math.min(
      15,
      Math.floor(
        this.width / (this.config.brickWidth + this.config.brickSpacing),
      ),
    );
    const totalWidth =
      cols * (this.config.brickWidth + this.config.brickSpacing) -
      this.config.brickSpacing;
    ...
    for (let row = 0; row < rows; row++) {
      for (let col = 0; col < cols; col++) {
        const x =
          startX + col * (this.config.brickWidth + this.config.brickSpacing);
        const y =
          startY + row * (this.config.brickHeight + this.config.brickSpacing);
        ...
        const hasGlassCoating =
          Math.random() < this.difficultySettings.glassCoatingChance;
  ...
  calculateFPS(currentTime) {
    ...
    if (currentTime - this.fpsLastTime >= 1000) {
      this.fps = Math.round(
        (this.frameCount * 1000) / (currentTime - this.fpsLastTime),
      );
      this.frameCount = 0;
  ...
  updateUpgradeEffects() {
    ...
    if (this.activeUpgradeEffects.chargedShot.startTime > 0) {
      const elapsedTime =
        currentTime - this.activeUpgradeEffects.chargedShot.startTime;
    ...
    if (this.activeUpgradeEffects.effectActivator.startTime > 0) {
      const elapsedTime =
        currentTime - this.activeUpgradeEffects.effectActivator.startTime;
    ...
    if (
      ...
      this.activeUpgradeEffects.chargedShot.maxCharge
    ) {
      this.activeUpgradeEffects.chargedShot.chargeLevel =
        this.activeUpgradeEffects.chargedShot.maxCharge;
    }
  ...
  changeBallDirection() {
    ...
    this.balls.forEach((ball) => {
      ...
      do {
        ...
        const isTooHorizontal =
          horizontalAngle < Math.PI / 22.5 || // < 8°
    ...
    if (this.ballEffects.zigzag) {
      this.ballEffects.zigzagPattern =
        this.ballEffects.zigzagPattern === 0 ? 1 : 0; // Alternar entre 0 e 1
  ...
  generateNewBricksOnRedHit() {
    ...
    for (let i = 0; i < numNewBricks; i++) {
      ...
      while (attempts < 50 && !positionFound) {
        ...
        positionFound = !this.bricks.some(
          (brick) =>
            !brick.destroyed &&
  ...
  restoreBricksOnRedPanic() {
    try {
      ...
      const destroyedBricks = this.bricks.filter(
        (brick) => brick && brick.destroyed,
      );
      ...
      for (let i = 0; i < bricksToRestore; i++) {
        ...
        if (brickToRestore) {
          ...
          this.createParticles(
            ...
            this.getBrickColorValue(brickToRestore.color),
          );

          // Se Pânico Vermelho estiver ativo SOZINHO e o bloco restaurado for azul,
          // 1/2 de chance de atirar fragmento
          if (
            this.phaseModifiers.redPanic &&
            !this.phaseModifiers.countdown &&
            brickToRestore.color === "blue" &&
            Math.random() < 0.5
          ) {
            this.createFragment(
              brickToRestore.x + brickToRestore.width / 2,
  ...
  updateBalls(deltaTime = 1 / 60) {
    this.balls.forEach((ball, index) => {
      ...
      if (
        this.hasUpgrade("risk_converter") &&
        this.riskConverterSpeedMultiplier
      ) {
      ...
      if (this.ballEffects.zigzag) {
        ...
        const zigzagReduction = this.hasUpgrade("zigzag_stabilizer")
          ? 0.8
          : 1.0;
        ...
        if (this.ballEffects.zigzagPattern === 0) {
          ...
          vx +=
            Math.sin(this.ballEffects.zigzagTimer * 0.3) *
            2.5 *
            zigzagReduction;
          vy +=
            Math.cos(this.ballEffects.zigzagTimer * 0.2) *
            0.8 *
            zigzagReduction;
        } else {
      ...
      if (ball.y > this.height) {
        ...
        if (
          this.activeUpgradeEffects.safetyNet.active &&
  ...
  updateFragments() {
    this.fragments.forEach((fragment, index) => {
      ...
      if (
        fragment.y + fragment.size > this.paddle.y &&
    ...
    this.powerUps.forEach((powerUp, index) => {
      ...
      if (powerUp.type === "charged_shot") {
        ...
        this.bricks.forEach((brick) => {
          if (
            !brick.destroyed &&
            brick.color !== "red" && // Não pode quebrar o núcleo
            ...
            if (!brick.hasGlassCoating || this.canBreakGlass("charged_shot")) {
              ...
              this.createParticles(
                ...
                this.getBrickColorValue(brick.color),
              );
        ...
        this.bricks.forEach((brick) => {
          if (
            ...
            // Só quebrar se não for bloco com vidro ou se tiver upgrade para quebrar vidro
            if (
              !brick.hasGlassCoating ||
              this.canBreakGlass("attached_cannons")
            ) {
              // Quebrar tijolo
              brick.destroyed = true;
              // Atualizar contador de tijolos
  ...
  checkCollisions() {
    this.balls.forEach((ball) => {
      ...
      if (ball.x - ball.radius <= 0 || ball.x + ball.radius >= this.width) {
        ...
        ball.x = Math.max(
          ball.radius,
          Math.min(this.width - ball.radius, ball.x),
        );
      }
  ...
  handleBrickCollision(ball, brick) {
    ...
    if (
      this.hasUpgrade("explosive_ball") &&
      (brick.color === "red" || brick.color === "yellow")
    ) {
    ...
    if (
      brick.color === "red" &&
      this.hasUpgrade("structural_damage") &&
    ...
    if (this.hasUpgrade("prime_ball") && this.isPrime(this.ballHitCount)) {
      const availableBricks = this.bricks.filter(
        (b) => !b.destroyed && b.color !== "red",
      );
      if (availableBricks.length > 0) {
        const randomBrick =
          availableBricks[Math.floor(Math.random() * availableBricks.length)];
        // Só quebrar se não for bloco com vidro ou se tiver upgrade para quebrar vidro
        if (!randomBrick.hasGlassCoating || this.canBreakGlass("prime_ball")) {
          ...
          this.createParticles(
            randomBrick.x + randomBrick.width / 2,
            randomBrick.y + randomBrick.height / 2,
            this.getBrickColorValue(randomBrick.color),
          );
    ...
    if (brick.color === "red") {
      ...
      if (!this.hasUpgrade("risk_converter")) {
        ...
        const availableBricks = this.bricks.filter(
          (b) => !b.destroyed && b.color !== "red" && b !== brick,
        );
        ...
        if (availableBricks.length > 0) {
          ...
          if (brick.isMoving) {
            ...
            if (brick.y < 0) {
              ...
              // Limitar à área superior da tela
              brick.y = this.height * 0.8 - brick.height;
            }
          }
    ...
    const hitLeft =
      ballPrevX + ball.radius <= brick.x && ball.x + ball.radius > brick.x;
    const hitRight =
      ballPrevX - ball.radius >= brick.x + brick.width &&
      ball.x - ball.radius < brick.x + brick.width;
    const hitTop =
      ballPrevY + ball.radius <= brick.y && ball.y + ball.radius > brick.y;
    const hitBottom =
      ballPrevY - ball.radius >= brick.y + brick.height &&
      ball.y - ball.radius < brick.y + brick.height;
    ...
    if (this.hasUpgrade("mirror_ball") && this.phaseTime <= 30) {
      ...
      const mirrorBrick = this.bricks.find(
        (b) =>
          !b.destroyed &&
          b.color !== "red" &&
      ...
      if (mirrorBrick) {
        ...
        if (!mirrorBrick.hasGlassCoating || this.canBreakGlass("mirror_ball")) {
          ...
          this.createParticles(
            mirrorBrick.x + mirrorBrick.width / 2,
            mirrorBrick.y + mirrorBrick.height / 2,
            this.getBrickColorValue(mirrorBrick.color),
          );
    ...
    if (
      this.hasUpgrade("reinforced_paddle") ||
      this.hasUpgrade("repulsor_shield")
    ) {
      ...
      const behindY =
        brick.y - this.config.brickHeight - this.config.brickSpacing;

      // Encontrar bloco de trás (linha de cima, mesma coluna)
      const behindBrick = this.bricks.find(
        (b) =>
          !b.destroyed &&
          b.color !== "red" &&
      ...
      if (behindBrick && !hitBrickIsMoving && !behindBrick.isMoving) {
        // Só quebrar se não for bloco com vidro ou se tiver upgrade para quebrar vidro
        if (
          !behindBrick.hasGlassCoating ||
          this.canBreakGlass("behind_breaker")
        ) {
          ...
          this.createParticles(
            behindBrick.x + behindBrick.width / 2,
            behindBrick.y + behindBrick.height / 2,
            this.getBrickColorValue(behindBrick.color),
          );
    ...
    this.bricks.forEach((targetBrick) => {
      if (!targetBrick.destroyed && targetBrick.color !== "red") {
        ...
        if (distance <= explosionRadius) {
          ...
          this.createParticles(
            targetBrick.x + targetBrick.width / 2,
            targetBrick.y + targetBrick.height / 2,
            this.getBrickColorValue(targetBrick.color),
          );
  ...
  selectNextPower() {
    if (this.activatablePowers.length > 0) {
      this.selectedPowerIndex =
        (this.selectedPowerIndex + 1) % this.activatablePowers.length;
  ...
  activateSpecificUpgrade(powerId) {
    switch (powerId) {
      case "super_magnet":
        if (
          !this.activeUpgradeEffects.superMagnet.active &&
          this.activeUpgradeEffects.superMagnet.startTime === 0
        ) {
      ...
      case "paddle_dash":
        if (
          !this.activeUpgradeEffects.paddleDash.active &&
          this.activeUpgradeEffects.paddleDash.startTime === 0
        ) {
      ...
      case "safety_net":
        if (
          !this.activeUpgradeEffects.safetyNet.active &&
          this.activeUpgradeEffects.safetyNet.startTime === 0
        ) {
      ...
      case "cushion_paddle":
        if (
          !this.activeUpgradeEffects.cushionPaddle.active &&
          this.activeUpgradeEffects.cushionPaddle.startTime === 0
        ) {
  ...
  activateUpgrade() {
    // Super Ímã
    if (
      this.hasUpgrade("super_magnet") &&
      !this.activeUpgradeEffects.superMagnet.active &&
      this.activeUpgradeEffects.superMagnet.timer <= 0
    ) {
      this.activeUpgradeEffects.superMagnet.active = true;
      this.activeUpgradeEffects.superMagnet.timer =
        this.activeUpgradeEffects.superMagnet.duration;
      this.createParticles(
        this.paddle.x + this.paddle.width / 2,
        this.paddle.y,
        "#3498db",
    ...
    // Tiro Carregado
    if (
      this.hasUpgrade("charged_shot") &&
      !this.activeUpgradeEffects.chargedShot.charging
    ) {
    ...
    // Rede de Segurança
    if (
      this.hasUpgrade("safety_net") &&
      !this.activeUpgradeEffects.safetyNet.active &&
      this.activeUpgradeEffects.safetyNet.timer <= 0
    ) {
      this.activeUpgradeEffects.safetyNet.active = true;
      this.activeUpgradeEffects.safetyNet.timer =
        this.activeUpgradeEffects.safetyNet.duration;
      this.createParticles(this.width / 2, this.height - 20, "#2ecc71");
    }
  }
  ...
  canDuplicateBall() {
    ...
    const purpleBalls = this.balls.filter(
      (ball) => ball.color === "purple",
    ).length;
  ...
  duplicateBall(originalBall) {
    ...
    const speed = Math.sqrt(
      originalBall.vx * originalBall.vx + originalBall.vy * originalBall.vy,
    );
  ...
  updateDifficultySettings() {
    ...
    const speedIncrease = Math.min((this.currentPhase - 1) * 0.02, 0.2);
    ...
    this.difficultySettings.paddleSizeMultiplier = Math.max(
      0.5,
      1.0 - paddleReduction,
    );
    ...
    this.difficultySettings.purpleBrickChance = Math.min(
      0.35,
      0.15 + densityIncrease * 0.01,
    ); // Máximo 20% acima do base
    this.difficultySettings.whiteBrickChance = Math.min(
      0.25,
      0.05 + densityIncrease * 0.02,
    ); // Máximo 20% acima do base

    // Película de vidro: A partir da fase 4, 15% de chance
    this.difficultySettings.glassCoatingChance =
      this.currentPhase >= 4 ? 0.15 : 0.0;

    // Tijolos Móveis: A partir da fase 5, 10% de chance por fileira
    this.difficultySettings.movingBricksChance =
      this.currentPhase >= 5 ? 0.1 : 0.0;
  ...
  updateSpeedDisplay() {
    ...
    // Atualizar displays
    document.getElementById("ballSpeedDisplay").textContent =
      Math.round(totalSpeed * 100) + "%";
    document.getElementById("baseSpeedDisplay").textContent =
      Math.round(baseSpeed * 100) + "%";
    document.getElementById("effectsSpeedDisplay").textContent =
      Math.round(effectsSpeed * 100) + "%";
  }
  ...
  applyDevUpgrade(upgradeId) {
    ...
    const isAlreadyActive = this.activeUpgrades.some(
  ...
  generateUpgradeOptions() {
    ...
    const shuffledUpgrades = [...availableUpgrades].sort(
      () => Math.random() - 0.5,
    );
    ...
    selectedUpgrades.forEach((upgrade) => {
      ...
      const priceColorClass = canAfford
        ? "price-affordable"
        : "price-expensive";
  ...
  getAvailableUpgrades() {
    ...
    const allUpgrades = [
      ...
      },
      {
        ...
        name: "Super Ímã",
        description:
          "Campo magnético que puxa a bolinha por 2 segundos (Mantenha espaço pressionado)",
        price: 180,
        type: "paddle",
        icon: this.getUpgradeSVG("super_magnet"),
      },
      {
        ...
        description:
          "Permite um movimento rápido (dash) para a esquerda ou direita por 3 segundos.",
        price: 140,
        type: "paddle",
        icon: this.getUpgradeSVG("paddle_dash"),
      },
      {
        ...
        description:
          "Diminui em 50% a velocidade de todas as bolinhas por 10 segundos. Cooldown de 60 segundos.",
        price: 80,
        type: "paddle",
        icon: this.getUpgradeSVG("cushion_paddle"),
      },
      {
        ...
        name: "Reforço",
        description:
          "A plataforma fica 2x mais alta e a bolinha destrói o bloco atingido e o bloco de trás",
        price: 220,
        type: "paddle",
      ...
      // Upgrades de Bolinha (8-14)
      {
        ...
        description:
          "A bolinha quebra 1 tijolo comum (azul) sem mudar de direção",
        price: 220,
        type: "ball",
        icon: this.getUpgradeSVG("piercing_ball"),
      },
...

      {
        ...
        name: "Multi-bola",
        description:
          "Cria uma nova bolinha grudada na plataforma. Liberada automaticamente em 2 segundos.",
        price: 200,
        type: "ball",
        icon: this.getUpgradeSVG("multi_ball"),
      },
      ...
      },
      {
        ...
        description:
          "Ativa efeito aleatório dos blocos na bolinha e ganha moedas baseadas na cor do efeito ativado",
        price: 60,
        type: "ball",
        icon: this.getUpgradeSVG("effect_activator"),
      },
      {
        ...
        description:
          "Quando a bolinha destrói um bloco, também destrói o bloco simetricamente posicionado do outro lado da tela (apenas nos primeiros 30 segundos de cada fase)",
        price: 250,
        type: "ball",
        icon: this.getUpgradeSVG("mirror_ball"),
      },
      {
        ...
        description:
          "A bolinha fica dourada e ganha +1 moeda extra por cada bloco quebrado",
        price: 150,
        type: "ball",
        icon: this.getUpgradeSVG("lucky_ball"),
      },
      {
        ...
        description:
          "Adiciona +1 moeda por bloco quebrado em combos e dobra a recompensa do combo máximo na loja",
        price: 200,
        type: "ball",
        icon: this.getUpgradeSVG("wombo_combo_ball"),
      },
      ...
      },
      {
        ...
        description:
          "Uma barreira de energia temporária aparece na parte inferior da tela por 15 segundos",
        price: 300,
        type: "utility",
        icon: this.getUpgradeSVG("safety_net"),
      },
      ...
      },
      {
        ...
        name: "Seguro de Vida",
        description:
          "Ao perder uma vida, ganha 100 moedas ao invés de perder 10",
        price: 150,
        type: "utility",
        icon: this.getUpgradeSVG("life_insurance"),
      },
      ...
      },
      {
        ...
        name: "Bolinha Prima",
        description:
          "A cada número primo de batidas, destrói um bloco aleatório (não vermelho)",
        price: 120,
        type: "special",
        icon: this.getUpgradeSVG("prime_ball"),
      },
      ...
      },
      {
        ...
        description:
          "Pausa todas as bolinhas por 3 segundos. Cooldown de 40 segundos.",
        price: 180,
        type: "ball",
        icon: this.getUpgradeSVG("time_ball"),
      },
      ...
      },
      {
        ...
        description:
          "Mantenha espaço pressionado (até 5s) para atravessar tijolos sem quebrá-los. Cooldown 60s.",
        price: 140,
        type: "ball",
        icon: this.getUpgradeSVG("dimensional_ball"),
      },
    ...
    if (this.removedInitialPower) {
      const exists = allUpgrades.some(
        (u) => u.id === this.removedInitialPower.id,
      );
      if (!exists) {
  ...
  selectUpgrade(upgrade, cardElement, discountedPrice = null) {
    ...
    if (
      this.removedInitialPower &&
      upgrade.id === this.removedInitialPower.id
    ) {
  ...
  continueToNextPhase() {
    ...
    const hasBoughtSomething =
      this.shopPurchases && this.shopPurchases.length > 0;
  ...
  updateUI() {
    ...
    if (upgradeSubtitle) {
      ...
      upgradeSubtitle.textContent = parts.length
        ? parts.join(" | ")
      ...
      if (priceElement) {
        ...
        if (!card.classList.contains("disabled")) {
          ...
          const originalPriceSpan =
            priceElement.querySelector(".original-price");
  ...
  updateBrickCounter() {
    ...
    document.getElementById("blueCount").textContent =
      this.currentBrickCount.blue;
    document.getElementById("yellowCount").textContent =
      this.currentBrickCount.yellow;
    document.getElementById("greenCount").textContent =
      this.currentBrickCount.green;
    document.getElementById("purpleCount").textContent =
      this.currentBrickCount.purple;
    document.getElementById("grayCount").textContent =
      this.currentBrickCount.gray;
    document.getElementById("whiteCount").textContent =
      this.currentBrickCount.white;
    document.getElementById("redCount").textContent =
      this.currentBrickCount.red;
  }
  ...
  createPowersInterface() {
    ...
    const activeUpgradesWithCooldown = activeUpgrades.filter((upgrade) =>
      this.hasCooldown(upgrade.id),
    );
    const activeUpgradesWithoutCooldown = activeUpgrades.filter(
      (upgrade) => !this.hasCooldown(upgrade.id),
    );

    // Separar inativos por tipo de cooldown
    const inactiveUpgradesWithCooldown = inactiveUpgrades.filter((upgrade) =>
      this.hasCooldown(upgrade.id),
    );
    const inactiveUpgradesWithoutCooldown = inactiveUpgrades.filter(
      (upgrade) => !this.hasCooldown(upgrade.id),
    );
  ...
  updatePowerSelectionUI() {
    ...
    if (this.lastActivatablePowersCount !== this.activatablePowers.length) {
      ...
      const powerItems = powerSelectionContainer.querySelectorAll(
  ...
  drawBrick(brick) {
    ...
    if (brick.color === "red" && brick.hits < brick.maxHits) {
      ...
      for (let i = 0; i < crackCount; i++) {
        this.ctx.beginPath();
        this.ctx.moveTo(
          brick.x + ((i + 1) * brick.width) / (crackCount + 1),
          brick.y,
        );
        this.ctx.lineTo(
          brick.x + ((i + 1) * brick.width) / (crackCount + 1),
  ...
  drawPowerUp(powerUp) {
    if (powerUp.type === "charged_shot") {
      ...
      glowGradient.addColorStop(
        0,
        `rgba(255, 255, 0, ${0.3 * pulseIntensity})`,
      );
      glowGradient.addColorStop(
        0.5,
        `rgba(255, 215, 0, ${0.2 * pulseIntensity})`,
      );
      ...
      glowGradient.addColorStop(
        0.5,
        `rgba(231, 76, 60, ${0.3 * pulseIntensity})`,
      );
